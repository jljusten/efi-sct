  ...
  )
{
  ...
  if  (IS_IP_FILTER_ZEROED(&BcInterface->Mode->IpFilter) == FALSE) {
    AssertionType = EFI_TEST_ASSERTION_FAILED;
    StandardLib->RecordMessage (
                   StandardLib,
                   EFI_VERBOSE_LEVEL_DEFAULT,
                   L"IS_IP_FILTER_ZEROED(Mode->IpFilter) - Fail\r\n"
                   );
  }
...
//  BOOLEAN                               OrigSendGUID, NewSendGUID;
  UINT8                                 OrigTTL, NewTTL;
//  UINT8                                 OrigToS, NewToS;
//  BOOLEAN                               OrigMakeCallback, NewMakeCallback;
